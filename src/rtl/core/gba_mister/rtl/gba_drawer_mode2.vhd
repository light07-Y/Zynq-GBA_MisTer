...
architecture arch of gba_drawer_mode2 is
   ...
   signal xxx_tile             : unsigned(6 downto 0) := (others => '0');
   signal yyy_tile             : unsigned(6 downto 0) := (others => '0');
   ...
   begin
      if rising_edge(clk100) then
         ...
         case (vramfetch) is
            ...
            when CALCADDR2 =>
               xxx_tile  <= unsigned(xxx(9 downto 3));
               yyy_tile  <= unsigned(yyy(9 downto 3));
               vramfetch <= CALCADDR3;

            when CALCADDR3 =>
               case (to_integer(screensize)) is
                  when 0 => tileindex_var := to_integer(yyy_tile(3 downto 0) & xxx_tile(3 downto 0)); -- 16 x 16
                  when 1 => tileindex_var := to_integer(yyy_tile(4 downto 0) & xxx_tile(4 downto 0)); -- 32 x 32
                  when 2 => tileindex_var := to_integer(yyy_tile(5 downto 0) & xxx_tile(5 downto 0)); -- 64 x 64
                  when 3 => tileindex_var := to_integer(yyy_tile(6 downto 0) & xxx_tile(6 downto 0)); -- 128 x 128
                  when others => null;
               end case;
