...
entity gba_savestates is
   ...
   (
      ...
      SAVE_BusAddr           : out    std_logic_vector(27 downto 0);
      ...
      bus_out_Adr            : out    std_logic_vector(25 downto 0) := (others => '0');
...
architecture arch of gba_savestates is
   ...
   signal internal_bus_done_q   : std_logic := '0';
   signal save_bus_done_q       : std_logic := '0';
   signal bus_out_done_q        : std_logic := '0';
   
   signal internal_databuffer   : std_logic_vector(63 downto 0) := (others => '0');
   signal first_dword           : std_logic := '0';
   signal SAVE_BusAddr_int      : std_logic_vector(27 downto 0) := (others => '0');
   signal bus_out_Adr_int       : std_logic_vector(25 downto 0) := (others => '0');
   
   signal header_amount         : unsigned(31 downto 0) := (others => '0');

begin 

   SAVE_BusAddr         <= SAVE_BusAddr_int;
   bus_out_Adr          <= bus_out_Adr_int;
   ...
   begin
      if rising_edge(clk100) then
         ...
         gb_on_1 <= gb_on;
         internal_bus_done_q <= internal_bus_out.done;
         save_bus_done_q     <= SAVE_BusReadDone;
         bus_out_done_q      <= bus_out_done;
         ...
         case state is
            ...
            when SAVE_WAITSETTLE =>
               if (bus_ena_in = '1') then
                  ...
                  first_dword          <= '1';
                  SAVE_BusRnW          <= '1';
                  bus_out_Adr_int      <= std_logic_vector(to_unsigned(savestate_address + HEADERCOUNT, 26));
                  bus_out_rnw          <= '0';
            ...
            when SAVEINTERNALS_WRITE => 
               if (bus_out_done_q = '1') then
                  bus_out_active <= '0';
                  bus_out_Adr_int <= std_logic_vector(unsigned(bus_out_Adr_int) + 2);
            ...
            when SAVEMEMORY_NEXT =>
               if (savetype_counter < SAVETYPESCOUNT) then
                  ...
                  SAVE_BusAddr_int <= savetypes(savetype_counter).addr;
                  SAVE_Bus_ena <= '1';
               else
                  state          <= SAVESIZEAMOUNT;
                  bus_out_Adr_int <= std_logic_vector(to_unsigned(savestate_address, 26));
            ...
            when SAVEMEMORY_READ =>
               if (save_bus_done_q = '1') then
                  if (first_dword = '1') then
                     first_dword               <= '0';
                     bus_out_Din(31 downto 0)  <= SAVE_BusReadData;
                     SAVE_BusAddr_int          <= std_logic_vector(unsigned(SAVE_BusAddr_int) + 4);
                     SAVE_Bus_ena              <= '1';
                  else
