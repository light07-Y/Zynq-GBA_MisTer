...
static BaseType_t PsUsbOsal_IsInsideInterrupt(void)
{
    return (ulPortInterruptNesting != 0U) ? pdTRUE : pdFALSE;
}
#define USB_OSAL_IN_ISR() (PsUsbOsal_IsInsideInterrupt() != pdFALSE)
#else
#define USB_OSAL_IN_ISR() (xPortIsInsideInterrupt() != pdFALSE)
#endif
