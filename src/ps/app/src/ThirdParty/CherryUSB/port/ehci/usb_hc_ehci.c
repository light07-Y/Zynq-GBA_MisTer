...
static inline bool ehci_urb_has_in_data(const struct usbh_urb *urb)
{
    uint8_t ep_type;

    if ((urb == NULL) || (urb->transfer_buffer == NULL) || (urb->transfer_buffer_length == 0U)) {
        return false;
    }

    ep_type = USB_GET_ENDPOINT_TYPE(urb->ep->bmAttributes);
    if (ep_type == USB_ENDPOINT_TYPE_CONTROL) {
        if (urb->setup == NULL) {
            return false;
        }
        return ((urb->setup->bmRequestType & 0x80U) != 0U);
    }

    return ((urb->ep->bEndpointAddress & 0x80U) != 0U);
}

static inline void ehci_urb_prepare_cache(struct usbh_urb *urb)
{
    size_t aligned_len;
    ...
    /* Zynq PS USB Host 这里走“Cache + 显式维护”方案：
     * SETUP 包必须先 flush，IN buffer 提交前先 invalidate，避免 EHCI
     * 读到旧 setup 或 CPU 读回陈旧数据，之前设备描述符读取就踩过这个坑。 */
    if (urb->setup != NULL) {
        usb_dcache_flush((uintptr_t)urb->setup,
                         USB_ALIGN_UP(sizeof(struct usb_setup_packet), CONFIG_USB_ALIGN_SIZE));
    }

    if ((urb->transfer_buffer == NULL) || (urb->transfer_buffer_length == 0U)) {
        return;
    }

    aligned_len = USB_ALIGN_UP(urb->transfer_buffer_length, CONFIG_USB_ALIGN_SIZE);
    if (ehci_urb_has_in_data(urb)) {
        usb_dcache_invalidate((uintptr_t)urb->transfer_buffer, aligned_len);
    } else {
        usb_dcache_flush((uintptr_t)urb->transfer_buffer, aligned_len);
    }
}

static inline void ehci_urb_complete_cache(struct usbh_urb *urb)
{
    if ((urb == NULL) || !ehci_urb_has_in_data(urb)) {
        return;
    }

    /* IN 方向完成后再 invalidate 一次，保证 CPU 看到的是 DMA 刚写回的报告。 */
...
static struct ehci_qh_hw *ehci_control_urb_init(struct usbh_bus *bus, struct usbh_urb *urb, struct usb_setup_packet *setup, uint8_t *buffer, uint32_t buflen)
{
    ...
    if (setup->wLength > 0) {
        ...
        /* 这里必须保留上面判断出的 PID 方向，不能再无脑 OR 一个 PID_OUT。
         * 之前这么做会让 GET_DESCRIPTOR 的 data stage 方向错误。 */
        token |= QTD_TOKEN_STATUS_ACTIVE |
                 QTD_TOKEN_TOGGLE |
                 ((uint32_t)EHCI_TUNE_CERR << QTD_TOKEN_CERR_SHIFT) |
...
static void ehci_check_qh(struct usbh_bus *bus, struct ehci_qh_hw *qhead, struct ehci_qh_hw *qh)
{
    ...
    if ((token & QTD_TOKEN_STATUS_ERRORS) == 0) {
        ...
        if (token & QTD_TOKEN_STATUS_BABBLE) {
            ...
            urb->data_toggle = 0;
        /* 某些失败场景里 HALTED 会和 MMF/XACTERR/DBERR 同时出现，
         * 这里优先上报真实链路错误，避免把总线问题误判成单纯 STALL。 */
        } else if (token & (QTD_TOKEN_STATUS_DBERR | QTD_TOKEN_STATUS_XACTERR | QTD_TOKEN_STATUS_MMF)) {
        ...
        USB_LOG_WRN("EHCI qtd token=0x%08x ep=0x%02x type=%u speed=%u dev=%u port=%u portsc=0x%08x err=%d\r\n",
                    (unsigned int)token,
                    urb->ep->bEndpointAddress,
                    (unsigned int)USB_GET_ENDPOINT_TYPE(urb->ep->bmAttributes),
                    (unsigned int)urb->hport->speed,
                    (unsigned int)urb->hport->dev_addr,
                    (unsigned int)urb->hport->port,
                    (unsigned int)EHCI_HCOR->portsc[urb->hport->port - 1],
                    urb->errorcode);
        if ((USB_GET_ENDPOINT_TYPE(urb->ep->bmAttributes) == USB_ENDPOINT_TYPE_CONTROL) &&
            (urb->setup != NULL)) {
            USB_LOG_WRN("EHCI setup bm=0x%02x req=0x%02x val=0x%04x idx=0x%04x len=%u\r\n",
                        (unsigned int)urb->setup->bmRequestType,
                        (unsigned int)urb->setup->bRequest,
                        (unsigned int)urb->setup->wValue,
                        (unsigned int)urb->setup->wIndex,
                        (unsigned int)urb->setup->wLength);
        }
    }

    ehci_qh_scan_qtds(bus, qhead, qh);
    ehci_urb_complete_cache(urb);
